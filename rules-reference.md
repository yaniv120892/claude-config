...
**Proof of Work Is Not Regression Coverage** — A throwaway verification script proves the change worked once, on the branch that shipped it, in the hands of the person who already knew what to look for. It proves nothing about the branch after it. The failure mode is specific and common: a behaviour expressed as a *deviation from a default* — a per-call timeout override, a non-default argument, a guard added at one call site — is a single line that reads as removable. Delete it and the code still compiles, the shared default silently takes over, and every committed test still passes, because the committed tests only ever covered the default. The reviewer who catches this is asking for the one thing a PoW script structurally cannot provide: a check that survives the merge and fails for the *next* person.
> Pattern: For any behaviour a later change could silently undo, commit a spec that drives the real code path (the actual service method, not a re-implementation of its call) and asserts the deviating value against the named constant, not a literal. Cover the shared default and the explicit override alongside it, so the spec documents the whole contract rather than one point on it. Then verify the spec bites: revert the change locally, watch that specific test fail, restore. A test you have not seen fail is a test you have not written.
> Avoid: A verification script written, run, pasted into the PR description as Proof of Work, and deleted. A test that only asserts the shared default the override deviates from — it passes identically whether the override exists or not. Asserting a raw literal (`40000`) where the constant is the contract.
> Why the PoW section still earns its place: it shows a reviewer that the behaviour was observed end to end, including things a unit spec cannot reach (a live upstream, real latency). It is evidence for the review, not coverage for the repo. Both, not either.
...
> Pattern: Squash-merge every PR you own and delete the branch. The squash message defaults to the PR title, so write that title as a conventional commit (`<type>(<scope>): <description>`) — that title, not any commit on the branch, becomes the permanent base-branch history, so the type and scope have to be right there. Pull request commands go through `gh`; the `pr-workflows` plugin owns those skills.
...
**Comments Name the Mechanism, Not Today's Provider** — a comment that names the current vendor is describing the deployment, not the code. The code is reacting to a *property* the vendor happens to have, and every other vendor with that property forces the same code. So the day the host changes, the comment is wrong while the code it sits on is still right — the worst failure mode a comment has, because a reader trusts it and there is nothing to compile against it.
> Pattern: Name the property. A comment reading "DATABASE_URL points at a transaction pooler, which does not hold the advisory lock `prisma migrate` takes" survives a move from one pooler to another, and it hands the reader the thing to verify — does this endpoint pool? — instead of a brand to look up.
> Avoid: The current host, region, plan tier, dashboard, or account naming scheme written beside code that would read identically on any other provider. The same applies to config: `.env.example`, Terraform, CI YAML and Helm values are where vendor names creep in most, because the value beside the comment really is provider-specific even when the *reason* is not.
> Exception: a workaround for one named product's own bug or quirk. There the vendor is the mechanism — `// Binance answers 451 to US-hosted requests` is not portable knowledge, it is the fact, and naming the product is what lets a reader retest whether it still holds.
> The deployment fact is worth writing down; it just belongs in one maintained place — `CLAUDE.md` or the README, where the rest of the infra is already named and where a PR that changes it is expected to update it — not restated in each file that reacts to it. Restating it is how three copies drift.
> Origin: a `schema.prisma` comment reading "DATABASE_URL is Neon's pooled endpoint", repeated in `.env.example` and `CLAUDE.md`. The advisory-lock behaviour is true of PgBouncer and RDS Proxy too; only the middle copy needed the brand.
...
> Pattern: POST/GET through the CLI's own API passthrough, which authenticates, refreshes, and resolves the host itself: `gh api <path> --method POST --input -`. Pipe the JSON body via stdin. Derive any host/repo from the CLI's own metadata output rather than hardcoding it.
> Avoid: `gh auth token` feeding an `Authorization` header on a hand-built `urllib`/`requests`/`curl` call. A token-read like this can return an empty string with **exit code 0**, so `check=True` won't catch it.
> Example: `subprocess.run(["gh","api",path,"--method","POST","--input","-"], input=json.dumps(payload))` — not `urllib.request.Request(url, headers={"Authorization": read_token()})`.
