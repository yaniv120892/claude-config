...
Re-read the findings against `~/.claude/CLAUDE.md` (and the repo's `.claude/rules/` if present). Common ones to self-check: braces on all control flow, no `as` casts, explicit class access modifiers, `T[]` over `Array<T>`, no unexplained comments, no `eslint-disable`. Fix violations.
