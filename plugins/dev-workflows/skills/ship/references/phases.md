...
Then open or update the PR. **REQUIRED SUB-SKILL:** `creating-prs` — it owns the `gh` invocation and the title/description gate. Push to the feature branch only.
...
**PR description: REQUIRED SUB-SKILL:** `writing-pr-description`. Write it against the **final** diff, after simplify — not the original plan. Pull the Proof of Work section from `<worktree>/.claude/ship/proof-of-work.md` — the evidence artifact, not the skill's own `references/proof-of-work.md`. If a description already exists, rewrite it rather than appending.
