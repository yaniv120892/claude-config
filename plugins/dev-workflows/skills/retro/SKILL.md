...
- **Navigation**: how easy was it to find the right files? Are there hidden dependencies between files? Would a pointer in a `CLAUDE.md` or rules file have saved the hunt?
- **Automated checks**: could lint, types, tests, or a hook have caught a mistake the agent made?
- **Coding standards**: should a rule be added, removed, or clarified? Route it through the **Memory Management Protocol** in `shared-rules.md`. *Use when* the same class of mistake slipped through review.
- **No-op instructions**: instructions in steering files (`shared-rules.md`, `rules/*.md`, a repo `CLAUDE.md`) that fail the **Delete No-Op Instructions** rule in `rules/authoring-docs.md`. *Use when* the steering files are large and unwieldy.
...
- **Information access**: logs or services the agent couldn't see — teeing dev-server logs, read-only access to a third-party service.
